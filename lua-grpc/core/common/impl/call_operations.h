...
#include "../../../core/common/impl/call_op_data.h"  // for CodSendInitMd
#include "../../../core/common/impl/metadata_vector.h"  // for MetadataVector
#include "../../../core/common/impl/slice.h"  // for SliceFromCopiedString()
...
class CallOperations GRPC_FINAL {
 ...
 public:
  ...
  inline void ServerSendStatus(grpc_metadata* trail_md, size_t trail_md_count,
                               const grpc_status_code& status_core,
...
void CallOperations::ClientRecvStatus(grpc_metadata_array* trailing_metadata,
  ...
  op.data.recv_status_on_client.status = status_core;
...
void CallOperations::ServerSendStatus(
    ...
    const grpc_status_code& status_core, const std::string& status_details) {
  assert(nops_ < MAX_OPS);
  grpc_op& op = ops_[nops_++];
  ...
  op.data.send_status_from_server.status = status_core;
